...
if (params.help) {
    ...
    println "\nARGUEMENTS:"
    println "--help       BOOLEAN   Show this menu (default: false)"
    println "-profile     STRING    Executor to be used (default: local)."
    println "                       Available options: [ slurm | standard ]."
    println "--mode       STRING    Specify which step of the workflow you are running (see https://github.com/h3abionet/h3ameta)."
    println "                       Available options:"
    println "\t\t\t\tprep.Containers      : <Description"
    println "\t\t\t\tprep.KrakenDB        : <Description"
    println "\t\t\t\tprep.BrakenDB        : <Description"
    println "\t\t\t\tprep.Indexes         : <Description"
    println "\t\t\t\trun.ReadQC           : <Description"
    println "\t\t\t\trun.ReadTrimming     : <Description"
    println "\t\t\t\trun.Classification   : <Description"
    println "\t\t\t\trun.StrainComp       : <Description"
    println "\t\t\t\trun.ViralDetectLong  : <Description"
    println "\t\t\t\trun.ViralDetectShort : <Description"
    println "--data       FOLDER    Path to where the input data (FASTQ files) is located (default: \$PWD)."
    println "                       Supported FASTQ files: [ fastq | fastq.gz | fastq.bz2 | fq | fq.gz | fq.bz2 ]"
    println "--out        FOLDER    Path to where the output should be directed (default: \$PWD/output)."
    println "--from       STRING    Specify to resume workflow from the QC or trimming step."
    println "                       Available options: [ run.ReadQC | run.ReadTrimming ]."
    println "--pairedEnd  BOOLEAN   If working with paired-end FASTQ files (default: true)."
    println "--singleEnd  BOOLEAN   If working with single-end FASTQ files (default: false)."
    println "--read_type  STRING    Specify type of long-read."
    println "                       Available options: [ nanopore | pacbio ]."
    println "\nDATABASES:"
    println "--kraken_db   FOLDER   <Description> (default: \$PWD/KDB)."
    println "--viral_db    FILE     <Description>"
    println "--decontam_db FILE     <Description>"
    println "--annot_db    FILE     <Description>"
    println "--acc_2_tax   FILE     <Description>"
    println "--names_dmp   FILE     <Description>"
    println "--dataset_table FILE   <Description>"
    println "\nPARAMETERS:"
    println "--trim       STRING    Parameters for Trimmomatic. See http://www.usadellab.org/cms/index.php?page=trimmomatic"
    println "                       The default parameters for Trimmomatic (for both paired- and single-end sequences):"
    ...
    println "--readlen    STRING    <Description>"
    println "--tax_level  STRING    <Description>"
